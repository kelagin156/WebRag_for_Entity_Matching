...
# Class to handle entity matching using OpenAI's GPT and Tavily for web search
class WebRAGEntityMatcher:
    def __init__(self, openai_api_key, tavily_api_key):
        self.openai_api_key = openai_api_key
        self.tavily_api_key = tavily_api_key

    # Method to match entities using OpenAI's GPT without WebRAG
    ...
    # Method to match entities using OpenAI's GPT with WebRAG
    ...
    # Method to search using Tavily and return results
    def search_with_tavily(self, title, mex_results):
    ...
    # Method to match entities using WebRAG with Tavily
    def webRag_entity_match(self, entity_1, entity_2, title1, title2):
        webrag_results = {}

        # Only one Travily query per side
        info_1_full = self.search_with_tavily(title1, 5)
        info_2_full = self.search_with_tavily(title2, 5)

        full_results_1 = info_1_full["results"]
        full_results_2 = info_2_full["results"]

        # For each n in [1, 3, 5], slice the results and build context
        for n in [1, 3, 5]:
            results_1_n = full_results_1[:n]
            results_2_n = full_results_2[:n]
            
            # Build context from the sliced results for ChatGPT prompt
            ...
            # Embedd all the results together
...
if __name__ == "__main__":
    ...
    TAVILY_key = "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"
    file_path = "final400datasets.json"
    matcher = WebRAGEntityMatcher(openai_api_key=API_key, travily_api_key=TAVILY_key)    
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        data = json.load(file)
        try:
            # Process all entity pair records
            for i, record in enumerate(data):
                if i >= 0: # in case the code fails the number can be adjusted to the last i
                    ...
                    # Convert baseline result to binary prediction
