        ...
        try:
            for i, record in enumerate(data):
                if i <= 2: # in case the code fails the number can be adjusted to the last i
                    ...
                    if "100%" in baseline_result: #If Chat GPT is sure then webrag is not n
                        webRag_y_pred = chatGPT40_mini_baseline_y_pred
                    else:
                        # WebRAG-enhanced entity matching
            ...
            df.to_csv("entity_matching_results_certain.csv", index=False, encoding="utf-8")
